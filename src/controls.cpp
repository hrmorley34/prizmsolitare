...
void Controls::SelectSrc()
{
    src = sel;
    // sel is left unchanged
}

Controls::Controls(Game *const game)
{
    this->game = game;
...
void Controls::ClearScreen() const
{
    DrawBackground(0, LCD_HEIGHT_PX, 0, LCD_WIDTH_PX);
}

void Controls::DrawScreenTop() const
{
    DrawBackground(
...
void Controls::DrawScreenTopShine(CardSelection sel) const
{
    if (sel.col >= 0)
        return;
...
void Controls::DrawScreenCol(int col) const
{
...
void Controls::DrawScreen() const
{
    DrawScreenTop();
...
void Controls::MoveSelection(const int dx, const int dy)
{
...
void Controls::HandleKey(const int key)
{
    switch (key)
    {
    case KEY_CTRL_UP:
